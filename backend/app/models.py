...
class Recipe(Base):
    ...
    image_candidates: Mapped[list[dict[str, Any]] | None] = mapped_column(JSON, default=list)
    image_candidate_index: Mapped[int | None] = mapped_column(Integer)
    image_lookup_status: Mapped[str | None] = mapped_column(String(40))
    image_lookup_reason: Mapped[str | None] = mapped_column(Text)
    image_lookup_attempt_count: Mapped[int] = mapped_column(Integer, default=0)
